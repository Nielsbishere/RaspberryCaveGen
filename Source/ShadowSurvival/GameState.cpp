...
//	bh->add("Health", Resources::loadTexture("../Resources/Textures/Bars.png", "Bars"), vec2(.5f - .125f, -1 + .125), vec3(.125, .25 / 3, .25), vec3(1, 1, 1), vec4(0, 70 / 256.f, 214 / 256.f, 105 / 256.f));
	...
	modelShader = Resources::loadShader("../Resources/Shaders/model.vsh", "../Resources/Shaders/model.fsh", "Model shader", 0b00);
	md2Shader = Resources::loadShader("../Resources/Shaders/md2.vsh", "../Resources/Shaders/md2.fsh", "MD2 shader", 0b00);
	billBoard = Resources::loadShader("../Resources/Shaders/billboard.vsh", "../Resources/Shaders/billboard.fsh", "Billboard shader", 0b00);
	caveShader = Resources::loadShader("../Resources/Shaders/terrain.vsh", "../Resources/Shaders/terrain.fsh", "Terrain shader", 0b00);
	...
	go->setFriction(.1);
	go->applyBehavior(new PlayerBehavior(bh, 30.f, 10.f, 10.f, 4.f, 5.f, 100.f));
...
//	go->setGravity(false);
	...
	for (u32 j = 0; j <= gh; j++){
		for (u32 i = 0; i <= gw; i++) {
			go = w->add(StringUtils::numstring(j * (gw + 1) + i, "Grid object #", ""), modelShader, Resources::loadModel("../Resources/Models/Sphere.obj", "Sphere"), Resources::loadTexture("../Resources/Textures/Dirt.png", "Dirt"), vec3((i / (gw / 2.f) - 1.f) * 50, 6.5, (j / (gh / 2.f) - 1.f) * 50), vec3(), vec3(1.f / gw * 25, 6, 1.f / gh * 25));
			go->setGravity(false);
