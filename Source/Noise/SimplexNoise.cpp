...
SimplexNoise::Grad::Grad(): x(0), y(0), z(0), w(0) {}
int SimplexNoise::fastfloor(double x) {
	int xi = (int)x;
	return x < xi ? xi - 1 : xi;
}
double SimplexNoise::dot(Grad g, double x, double y) {
	return g.x * x + g.y * y;
}
double SimplexNoise::dot(Grad g, double x, double y, double z) {
	return g.x * x + g.y * y + g.z * z;
}
double SimplexNoise::dot(Grad g, double x, double y, double z, double w) {
	return g.x * x + g.y * y + g.z * z + g.w * w;
}
SimplexNoise::SimplexNoise(): grad3 {
	Grad(1, 1, 0), Grad(-1, 1, 0), Grad(1, -1, 0), 
...
double SimplexNoise::tnoise(double x, double scale, int octaves, double persistance, double lacunarity) {
	double freq = 1, amp = 1;
	double y = 0;
...
double SimplexNoise::tnoise(double x, double y, double scale, int octaves, double persistance, double lacunarity) {
	double freq = 1, amp = 1;
	double z = 0;
	for (int i = 0; i < octaves; i++) {
...
double SimplexNoise::tnoise(double x, double y, double z, double scale, int octaves, double persistance, double lacunarity) {
	double freq = 1, amp = 1;
	double w = 0;
	for (int i = 0; i < octaves; i++) {
		w += (noise(x * scale * freq, y * scale * freq, z * scale * freq) + 1) / 2 * amp;
		amp *= persistance;
		freq *= lacunarity;
	}
	return z / octaves;
}
double SimplexNoise::tnoise(double x, double y, double z, double t, double scale, int octaves, double persistance, double lacunarity) {
	double freq = 1, amp = 1;
	double v = 0;
	for (int i = 0; i < octaves; i++) {
		v += (noise(x * scale * freq, y * scale * freq, z * scale * freq, t) + 1) / 2 * amp;
		amp *= persistance;
		freq *= lacunarity;
	}
	return z / octaves;
