...
namespace osomi{
	...
	private:
		...
		#endif
	protected:
		Mesh(std::string name, GLuint _vbo, GLuint _ibo, u32 _vertLength, u8 _indexPtrLength, u32 _vertices, AABB aabb): RenderObject(name, aabb), vbo(_vbo), ibo(_ibo), indexPtrLength(_indexPtrLength), vertices(_vertices), vertLength(_vertLength), chunks(nullptr){}
